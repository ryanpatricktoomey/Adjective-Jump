...
AJ.MenuScene = class MenuScene extends Phaser.Scene {
  ...
  _showPracticeMenu() {
    ...
    panel.fillRoundedRect(W / 2 - 165, 100, 330, this.scale.height - 140, 20);
    panel.lineStyle(2, 0xff6b9d, 0.6);
    panel.strokeRoundedRect(W / 2 - 165, 100, 330, this.scale.height - 140, 20);

    this.add.text(W / 2, 130, '연습 모드 선택', {
    ...
    AJ.PRACTICE_CONFIGS.forEach((cfg, i) => {
      const y = 180 + i * 55;
      this._makePracticeBtn(W / 2, y, cfg, overlay, panel);
    });

    // Close
    const close = this.add.text(W / 2, this.scale.height - 60, '✕ 닫기', {
      fontSize: '16px', fontFamily: 'Segoe UI, Arial, sans-serif', color: '#888888',
    }).setOrigin(0.5).setInteractive({ useHandCursor: true });
    close.on('pointerdown', () => {
      overlay.destroy(); panel.destroy(); close.destroy();
    });
  }

  _makePracticeBtn(x, y, cfg, ...destroyTargets) {
    const w = 290, h = 44;
    const btn = this.add.container(x, y);
    const bg  = this.add.graphics();
    bg.fillStyle(0x1a1a3a, 1);
    bg.fillRoundedRect(-w / 2, -h / 2, w, h, 10);
    bg.lineStyle(1.5, 0xff6b9d, 0.5);
    ...
    btn.on('pointerdown', () => {
      AJ.audio.playMenuClick();
      destroyTargets.forEach(d => d.destroy());
      btn.destroy();
      const levelConfig = {
